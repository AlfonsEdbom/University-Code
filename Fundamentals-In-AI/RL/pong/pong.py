    ...
    for gamma in GAMMA_LIST:
        for epsilon_decay in EPSILON_DECAY_LIST:
            for alpha in ALPHA_LIST:
                ...
                # Initialize agents
                my_agent = Agent(0, env.action_space[0].n, env.observation_space[0].shape, gamma, epsilon=0.95, min_epsilon=0.10, epsilon_decay=epsilon_decay, alpha=alpha)
                agents = [
                    my_agent,
                    RandomAgent(1)
                ]
                ...
                for ep_i in range(args.episodes):
                    ...
                    #plt.pause(0.1)
                print(f"Using gamma = {gamma}, epsilon decay = {epsilon_decay}, alpha = {alpha}: Got Wins - losses: {sum(wins) - sum(losses)} ")
