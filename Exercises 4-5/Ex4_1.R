...
pacman::p_load(pacman, here, Biostrings, HMM, seqinr)
...
#Define parameters for HMM
States <- c("H_GC", "H_AT")
Symbols <- c("A", "T", "C", "G")
start <- c(.5, .5)
trans <- matrix(c(.75, .25, .25, .75), 2)
emission <- matrix(c(.05, .45, .05, .45, .45, .05, .45, .05), 2)
...
#Train HMM to find optimal parameter settings
obs <- unlist(getSequence(virus_seq))
HMM_4new <- baumWelch(HMM_4state, obs, 20, delta=1E-9)
